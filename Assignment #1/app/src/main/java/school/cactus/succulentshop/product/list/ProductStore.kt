...
class ProductStore {
    ...
    fun relatedProducts(productId: Int): List<Product> {
        val relatedProductsSize = products.size / 2  //The related products size will be 1/2 of the products
        val relatedProducts = mutableListOf<Product>()

        //Select randomly from the products, and add to related products list
        for (i in 1..relatedProductsSize)
            products[(products.indices).random()].apply {
                if (this.id != productId && !relatedProducts.contains(this))
                    relatedProducts.add(this)
            }
